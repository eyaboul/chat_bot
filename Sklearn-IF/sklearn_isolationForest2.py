...
# Compteur pour le nombre total de requêtes à l'API de prédiction Isolation Forest
IF_PREDICT_REQUESTS_TOTAL = Counter(
    'if_predict_requests_total', 'Total requests to the Isolation Forest /predict endpoint'
)
# Histogramme pour la latence de l'opération de prédiction Isolation Forest
IF_PREDICT_LATENCY_SECONDS = Histogram(
    'if_predict_latency_seconds', 'Latency of Isolation Forest prediction in seconds'
)
# Compteur pour le nombre d'erreurs lors de la prédiction Isolation Forest
IF_PREDICT_ERRORS_TOTAL = Counter(
    'if_predict_errors_total', 'Total errors during Isolation Forest prediction'
)
# Jauge pour le nombre d'anomalies détectées lors de la dernière exécution (à mettre à jour)
IF_LAST_ANOMALIES_COUNT = Gauge(
    'if_last_anomalies_count', 'Number of anomalies detected in the last Isolation Forest run'
)
# Jauge pour la proportion d'anomalies détectées lors de la dernière exécution
IF_LAST_ANOMALIES_PERCENT = Gauge(
    'if_last_anomalies_percent', 'Percentage of anomalies detected in the last Isolation Forest run'
)
# Histogramme pour la durée d'entraînement du modèle Isolation Forest
IF_TRAINING_LATENCY_SECONDS = Histogram(
    'if_training_latency_seconds', 'Latency of Isolation Forest model training in seconds'
)
