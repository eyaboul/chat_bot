...
# Compteur pour le nombre total de requêtes à l'API de vectorisation
SPACY_REQUESTS_TOTAL = Counter(
    'spacy_vectorize_requests_total', 'Total requests to the /vectorize endpoint'
)
# Histogramme pour la latence de l'opération de vectorisation
SPACY_VECTORIZE_LATENCY_SECONDS = Histogram(
    'spacy_vectorize_latency_seconds', 'Latency of log vectorization in seconds'
)
# Compteur pour le nombre d'erreurs lors de la vectorisation
SPACY_VECTORIZE_ERRORS_TOTAL = Counter(
    'spacy_vectorize_errors_total', 'Total errors during log vectorization'
)
# Jauge pour indiquer si le modèle spaCy est chargé (1 si oui, 0 si non)
SPACY_MODEL_LOADED = Gauge(
    'spacy_model_loaded', 'Status of the spaCy model (1=loaded, 0=not loaded)'
)
# Compteur pour le nombre de lignes nettoyées par la fonction clean
SPACY_CLEANED_ROWS_TOTAL = Counter(
    'spacy_cleaned_rows_total', 'Total number of rows processed by clean function'
)
app = Flask(__name__)
